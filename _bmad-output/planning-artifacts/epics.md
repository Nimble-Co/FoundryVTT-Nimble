...
# dev-clean - Epic Breakdown

## Overview

This document provides the complete epic and story breakdown for dev-clean, decomposing the requirements from the PRD, UX Design, and Architecture into implementable stories.
...
**From Architecture:**
- Chat card state machine: cards progress through 5 states (Fresh → Awaiting Reaction → Ready to Apply → Applied → Undone), derived from data fields not stored enum
- Defend reaction data flow: cross-player interaction on a single chat message requiring permission model investigation (socket vs ownership vs custom handler)
- Undo/snapshot pattern: generalized apply-then-undo pattern for both damage and healing, storing HP snapshots on the message document
- GM Helper operates as dockable ApplicationV2 (standalone or sidebar), with context-aware modes (out-of-combat: world NPCs, in-combat: combatants only)
- Action economy tracked on combatant document (actionsTotal, actionsUsed, reactionsUsedThisRound), not actor level
- Variable action count: base 3 for heroes but modified by initiative, class features, effects. Reactions cost 1 action deducted from next turn.
- New chat card nodes added via effect tree pattern, not new card types
- New automation rule types registered via `CONFIG.NIMBLE.ruleDataModels`
- All reactive UI accesses Foundry documents through `.reactive` accessor
- No barrel exports — direct imports only

**From UX Design:**
- Chat cards use progressive disclosure: roll result → reaction decision → damage feedback, evolving in place
- Role-gated rendering: player buttons hidden from GM, GM buttons hidden from players (hidden, not disabled)
- Button hierarchy: one primary action per context, secondary for corrections (undo, re-roll)
- "Fast action + easy undo > slow confirmation" philosophy — no confirmation modals for apply/undo
- Defend reaction prompt shows exact math (incoming damage, armor value, result if defending, action cost) for informed tactical decisions
- Condition nodes render as clickable pills with rich tooltips; left-click applies to selected tokens
- Saving throw nodes show pass/fail branches with child effect nodes (damage, conditions)
- Target management: "+ Selected" and "◎ Targeted" buttons, per-target remove, hover highlights token on canvas
- Crit chain: progressive build with "Roll Again!" button, running total, visual intensity increase
- Character sheet: stats bar + resource bars + action economy pips always visible above tabs
- GM Helper: dense creature rows (1 row per creature), expandable for full actions, bulk operations at section level
- Accessibility: WCAG AA where practical, semantic HTML, ARIA labels on icon-only buttons, `prefers-reduced-motion` respected
- Color independence: status info (hit/miss/crit) never relies on color alone — always paired with text labels

### FR Coverage Map

| FR | Epic | Description |
|---|---|---|
| FR1 | Epic 1 | Character creation process |
| FR2 | Epic 1 | Character sheet editing |
| FR3 | Epic 1 | Level-up dialog |
| FR4 | Epic 1 | Portrait/token art separation |
| FR5 | Epic 1 | Sheet lock/unlock |
| FR6 | Epic 1 | Inventory management |
| FR7 | Epic 1 | Active effects/conditions display |
| FR8 | Epic 1 | Spell list management |
| FR9 | Epic 2 | Monster/NPC sheet creation |
| FR10 | Epic 3 | Activation dialog with roll options |
| FR11 | Epic 3 | Automatic action/mana deduction |
| FR12 | Epic 3 | Manual crit dice with exploding |
| FR13 | Epic 3 | Hit/miss/crit determination |
| FR14 | Epic 3 | GM attack/ability initiation |
| FR15 | Epic 3 | Initiative rolling |
| FR16 | Epic 3 | Effect application to targets |
| FR17 | Epic 4 | Chat card roll result display |
| FR18 | Epic 4 | GM apply damage from chat |
| FR19 | Epic 4 | GM apply healing from chat |
| FR20 | Epic 4 | Player chat card interactions |
| FR21 | Epic 4 | Role-based action display |
| FR22 | Epic 4 | Primary die color distinction |
| FR23 | Epic 5 | Combat encounter management |
| FR24 | Epic 5 | Visual carousel turn order |
| FR25 | Epic 5 | Side-based initiative sorting |
| FR26 | Epic 5 | Solo boss turn interleaving |
| FR27 | Epic 5 | Add/remove combatants |
| FR28 | Epic 5 | Minion turn grouping |
| FR29 | Epic 5 | Action tracking per combatant |
| FR30 | Epic 5 | Configurable tracker position |
| FR31 | Epic 6 | Compact creature overview |
| FR32 | Epic 6 | Execute actions from GM Helper |
| FR33 | Epic 6 | Feat synergy reminders |
| FR34 | Epic 6 | Minion group splitting |
| FR35 | Epic 6 | Minion batch resolution |
| FR36 | Epic 7 | Core compendium content |
| FR37 | Epic 7 | Drag-drop compendium |
| FR38 | Epic 7 | Homebrew content creation |
| FR39 | Epic 7 | Code-free content addition |
| FR40 | Epic 7 | Safe Rest recovery |
| FR41 | Epic 7 | Field Rest hit dice spending |
| FR42 | Epic 7 | Module event hooks |
| FR43 | Epic 7 | Tokenizer compatibility |
| FR44 | Epic 7 | System settings configuration |
| FR45 | Epic 8 | Automation building blocks |
| FR46 | Epic 8 | Dice pool management |
| FR47 | Epic 8 | Resource counter tracking |
| FR48 | Epic 8 | Toggle feature effects |
| FR49 | Epic 8 | Ad hoc condition application |
| FR50 | Epic 8 | Minion summoning |
| FR51 | Epic 8 | Shapeshift via toggle |
| FR52 | Epic 8 | Pre-roll parameter editing |
| FR53 | Epic 8 | Post-roll result editing |
| FR54 | Epic 8 | Per-die manipulation |
| FR55 | Epic 8 | Nimble-specific dice mechanics |
