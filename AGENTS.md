...
- **Never hardcode `'nimble'` as the system id**: Always import `SYSTEM_ID` from `#system` (or `SYSTEM_PATH` for `systems/<id>/...` asset paths). The dev rolling release rebuilds under a different system id (`nimble-dev`), and hardcoded `'nimble'` literals silently break flags, settings, sheet registration, and pack UUIDs in that build. This applies to:
  ...
  - **Audited exceptions:** `game.nimble` (our own arbitrary API namespace on `game`, assigned and read with the same literal key — self-consistent, not id-validated, and referenced by persisted macro command strings) and `Compendium.nimble.*` source ids in `src/migration/**` intentionally stay literal. Note that `dev-rebrand.mjs` does **not** rewrite them — it only globs `packs/**/*.json` — so on the dev build these literals will not match an actor's stored `Compendium.nimble-dev.*` ids. A migration that must resolve them on both installs has to normalize the prefix itself (see `Migration035AncestryBonusSplit`); one that only matches a *snapshot* of past data can leave them alone. The `systemId.test.ts` guard exempts these; new exceptions need an inline `// allow-hardcoded-system-id` comment with justification.
