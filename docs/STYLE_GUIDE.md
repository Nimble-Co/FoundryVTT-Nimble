...
| `calculateInventorySlotCost()`, `getContainerUsedCapacity()`, `findContainerStorageRejection()` | `src/utils/inventoryContainers.ts` | Container inventory rules: what a set of carried objects costs once each container's rule is applied, how much of a container's capacity is used, and why a container refuses an object |
