...
| `calculateInventorySlotCost()`, `getBaseSlotCost()`, `applyContainerSlotRule()`, `containerWaivesSmallObjectCost()`, `getContainerUsedCapacity()`, `findContainerStorageRejection()` | `src/utils/inventoryContainers.ts` | Container inventory rules: what a carried object costs on its own, how the container holding it changes that cost, how much of a container's capacity is used, and why a container refuses an object |
