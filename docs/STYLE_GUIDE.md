...
| `findArmedMovementOffer()` | `src/utils/movement/findArmedMovementOffer.ts` | The Movement Offer a token is carrying, rebuilt from the newest chat card that offers it one and not yet resolved; null when the Movement Offers toggle is off |
| `movementOfferConstraints()` | `src/utils/movement/movementOfferConstraints.ts` | The limit Foundry applies to a drag under an offer: cost for a Free Move that honours difficult terrain, distance otherwise, plus what the token already moved when the drop is measured against the Movement history |
| `buildCardMovementOffer()`, `cardMoveRecipients()` | `src/utils/movement/buildCardMovementOffer.ts` | Rebuild a `move` node's offer for one recipient from the chat card itself, and list the tokens a node applies to |
