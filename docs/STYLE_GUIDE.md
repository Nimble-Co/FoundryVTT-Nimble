...
| `getSpellSchoolLabel()`, `getSpellTierLabel()` | `src/utils/spellLabels.ts` | Localized display names for a spell school and a spell tier, falling back to the raw id and to `Tier N` / `Cantrip`. Two older variants survive on purpose — `compendiumSpellsFilter.ts` title-cases an unknown school and `SpellCard.svelte.ts` localizes its fallback — because converging them would change what those screens render |
