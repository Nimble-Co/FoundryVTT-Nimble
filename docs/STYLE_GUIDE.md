...
| `getSpellSchoolLabel()`, `getSpellTierLabel()` | `src/utils/spellLabels.ts` | Localized display names for a spell school and a spell tier (`Cantrip`, `Tier 3`), falling back to the raw school id and the bare tier number when `CONFIG.NIMBLE` carries no label. Two older variants survive on purpose — `compendiumSpellsFilter.ts` title-cases an unknown school and `SpellCard.svelte.ts` localizes its fallback — because converging them would change what those screens render |
| `knowsSpellSchool()` | `src/utils/knowsSpellSchool.ts` | Whether an actor holds any spell of a given school, at any tier. Drives the spell-scroll Arcana exemption |
| `createScrollFromSpell()`, `getSpellScrollData()` | `src/utils/createScrollFromSpell.ts` | Inscribe a spell onto a small-sized consumable scroll object, and read the scroll flag back off an item |
| `getSpellScrollTemplateTier()` | `src/utils/getSpellScrollTemplateTier.ts` | Tier of a dropped generic spell scroll pack blank (`Spell Scroll, Cantrip`, `Spell Scroll, Tier 1` … `Tier 9`), or `null` when the item is not one. Matches on the bare document id — identical across the `nimble` and `nimble-dev` builds — rather than a flag, `system.identifier`, or a Babele-translated name |
...
| `collectOptionPoolRequirements()`, `buildOptionPoolKey()` | `src/utils/collectOptionPoolRequirements.ts` | What each level from 1 to N asks a class to pick from an option pool (a set of feature groups, distinct from a charge pool), replaying the class feature index; a level offering a choice between alternatives is folded into one requirement drawing from every alternative. `buildOptionPoolKey` is the order-independent identity of a pool's groups |
| `collectOptionPoolCandidates()` | `src/utils/collectOptionPoolCandidates.ts` | Every feature in the index belonging to a set of groups, across all levels, deduplicated by uuid |
| `collectOptionPoolEntitlement()` | `src/utils/collectOptionPoolEntitlement.ts` | What a class grants from each option pool at a level, independent of the sheet: pools merged where they share a group, the granted count, one slot per level with the groups that level named, every candidate, and the members marked repeatable from the granting rule's `allowDuplicate` |
| `collectHeldPicks()` | `src/utils/collectHeldPicks.ts` | The picks a character holds per pool and member: feature items whose source is a pool member, namespaces folded, copies counted. The history is read only to order copies, tracked first |
| `collectSwappableOptions()` | `src/utils/collectSwappableOptions.ts` | The option pools a character may re-pick from on a rest: the entitlement plus what the sheet holds of each member, dropping only a pool with a single candidate |
| `resolveOptionSwapOffer()` | `src/utils/resolveOptionSwapOffer.ts` | Read a character's `optionSwap` and `skillPointMove` rules for a rest trigger into one offer: the groups covered (`null` for all), skill points to move, and one record per offering feature with its uuid, image, the groups it covers, and the points it moves |
| `planOptionSwap()`, `isSelectionApplicable()` | `src/utils/planOptionSwap.ts` | Turn a selection per pool (a multiset of member uuids) into item ids to delete and uuids to grant as a count difference per member, releasing from the front of each held list; a selection is applied as a trade at the held count or as a fill up to the granted count, and a grant inherits the history entry of the pick it replaces or `-1` when none names it |
...
| `reconcileMovementOffers()`, `findArmedMovementOffer()`, `settleMovementOffer()`, `movementOfferOutcome()` | `src/utils/movement/movementOffers.ts` | The Movement Offer module. Stamp a card's offers once per `move` node and recipient (at card creation and when targets change), find the open offer a token carries (null when the Movement Offers toggle is off), settle it after a Movement, and report its outcome for the card |
| `postMovementOfferCard()` | `src/utils/movement/postMovementOfferCard.ts` | Post a Movement Offer that no item activation made: a `movementOffer` card with one synthetic `move` node, born with its offers so the rest of the offer module works on it unchanged. Posts nothing without a recipient |
| `postMovementTriggerCard()` | `src/utils/movement/postMovementTriggerCard.ts` | Post a `movementTrigger` card naming the tokens a finished Movement found, as a card that lets the owner use the item on them or as a reminder |
| `matchMovementTrigger()` | `src/utils/movement/matchMovementTrigger.ts` | Whether a finished Movement fires a trigger, and which tokens it found: by event (own or another creature's Movement), relation by disposition, movement kind, spaces this Movement or this turn, and geometry (ends adjacent, enters or leaves Reach, passes through, moves toward) |
| `reconcileMovementContext()` | `src/utils/movement/movementContext.ts` | Stamp an activation card once with the Spaces Moved This Turn of the creature that used it, and each target's distance from the speaker token when it is added; notes read them through placeholders |
| `hasRuleCharge()`, `spendRuleCharge()` | `src/utils/chargePool/ruleChargeGate.ts` | Let a rule fire only while a charge pool holds a charge, and spend one when it fires; no limit for an empty identifier or with resource spending automation off |
| `markWaypointsPastOffer()` | `src/utils/movement/markWaypointsPastOffer.ts` | Mark the part of a planned drag past a Movement Offer so the token ruler draws it as core draws an unreachable path; never shortens the dropped path |
| `resolveArmedMovementOffer()` | `src/utils/movement/resolveArmedMovementOffer.ts` | Records on its card what came of an offer once the token finished moving: the spaces covered for a Movement of the offered kind, unused for any other. Runs on the primary active GM |
| `resolveMoveDistance()` | `src/utils/movement/resolveMoveDistance.ts` | A `move` node's distance in spaces for one recipient: the size override for the recipient's size or the formula, evaluated against the feature user's roll data with `@speed` as the recipient's walk speed; floored, never negative, 0 when unreadable |
...
| `effectiveVariants()`, `offersVariantChoice()`, `canonicalVariant()`, `addVariant()`, `removeVariant()` | `src/utils/ancestryVariants.ts` | An ancestry's variant names — the kinds of people it covers ("Dryad", "Shroomling"). Normalizes a stored list (trims, drops blanks, dedupes case-insensitively, keeps authored order), reports whether two or more remain so character creation offers a choice, resolves a submitted name to the ancestry's own spelling (or `null` when it offers no such name), and adds/removes one for the sheet's authoring control |
