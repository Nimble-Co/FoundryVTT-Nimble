...
| `applyCasterConcentration()`, `requiresConcentration()` | `src/utils/applyCasterConcentration.ts` | Whether an activated item carries the `concentration` property, and the shared path that puts the `concentration` condition on its owner at activation. Ends the caster's previous concentration first, so a second concentration spell leaves exactly one. Read from the property rather than from rules authored on the item, so homebrew spells and spell scrolls work with no authoring |
