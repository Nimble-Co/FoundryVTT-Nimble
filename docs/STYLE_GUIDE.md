...
| `requestMove()`, `selectMovingUser()`, `PLAN_MOVE_QUERY` | `src/utils/movement/requestMove.ts` | Offer a card's constrained drag to whoever should perform it: this user when they own the token, else an active owning player, else the active GM; plans locally or asks the owning client through a Foundry user query that carries only the card, node and token ids |
| `planOfferedMove()` | `src/utils/movement/planOfferedMove.ts` | On the dragging client: `Token#planMovement` limited to the offered spaces (cost for a free move that honours terrain, distance otherwise; one straight drag for a push "away" or "toward", a system convenience the rules do not dictate), then report how far the drop went; declined on dismiss, unavailable when this client cannot drag |
| `buildCardMovementOffer()`, `canUserTakeMovementOffer()`, `cardMoveRecipients()` | `src/utils/movement/buildCardMovementOffer.ts` | Rebuild a `move` node's offer for one recipient from the chat card itself, so relayed requests never carry a distance; the GM, the card's author and the token's owner may take it |
| `takeMovementOffer()` | `src/utils/movement/takeMovementOffer.ts` | The card button's action: request the drag, then stamp the result on the card (direct for GMs, relayed to the active GM over the system socket otherwise, see `movementOfferRelay.ts`) |
