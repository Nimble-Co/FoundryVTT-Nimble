...
| `collectOptionPicks()` | `src/utils/collectOptionPicks.ts` | The picks a character's level-up history records, per compendium source: each granted item id whose live item still carries a source, oldest granting level first. An item no entry names is not a pick |
| `collectSwappableOptions()` | `src/utils/collectSwappableOptions.ts` | The option pools a character may re-pick from on a rest: pools merged where they share a group, granted alternatives folded in as members, pick ids per member from the history (two ids under one source are two picks), members marked repeatable from the granting rule's `allowDuplicate`, compendium namespaces folded before matching |
