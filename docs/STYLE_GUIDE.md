...
| `applyConditionToActor()` | `src/utils/applyConditionToActor.ts` | The one path every automated condition application goes through: dedupes against a condition the target already has, fires the blocking `preApplyCondition` hook that condition immunity listens on, creates the effect with a recorded `origin` (the causing item, falling back to the causing actor) and an optional duration, then fires `conditionApplied`. `Actor#toggleStatusEffect` cannot record an origin, so this mirrors its internals instead |
