...
| `postMovementTriggerCard()` | `src/utils/movement/postMovementTriggerCard.ts` | Post a `movementTrigger` card naming the tokens a finished Movement found, as an offer to use the item on them or as a reminder |
...
| `reconcileMovementContext()` | `src/utils/movement/movementContext.ts` | Stamp an activation card with the user's Spaces Moved This Turn once, and each target's distance from the speaker token when it is added; notes read them through placeholders |
