...
| `applyCasterConcentration()` | `src/utils/applyCasterConcentration.ts` | Puts the `concentration` condition on an activated item's owner, and reports the effect it created so the caller can say whether it took. Ends the caster's previous concentration first, so a second concentration spell leaves exactly one. Reads the `concentration` property rather than rules authored on the item, so homebrew spells and spell scrolls work with no authoring |
