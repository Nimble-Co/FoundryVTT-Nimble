...
- **Ancestry variant names are not translated.** An ancestry that covers more than one kind of people lists those names in `system.variants` (`["Dryad", "Shroomling"]`). The generator does not extract that field and `mappings.json` has no converter for it, so character creation offers the English names in every language, and the ancestry on the finished character takes the English name the player picked. Translating them means adding `system.variants` to the extractor plus a `mapping` entry with its own converter, then regenerating every skeleton. `rules` is the closest precedent, but it keys by `rule.id` (falling back to `index:N`); a bare string array has no id to key on, so it would need an index-keyed scheme of its own.
- **Stale reporting is entry-level only.** When the regenerator detects an entry whose top-level `name` no longer matches any English source, it flags it in the build log. Stale *fields* inside a kept entry (e.g., a spell that previously had `costDetails` but no longer does) are silently retained — no log line, no diff marker. If you remove a field from a source document and want the orphaned translation cleaned up, edit the skeleton file by hand.
