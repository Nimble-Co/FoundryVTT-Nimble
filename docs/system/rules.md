...
- The target's flat reduction, resistance and monster heavy armor all resolve against each packet separately, which is what the engine already does everywhere damage arrives as more than one node. Two of those are wrong against the books and neither is introduced here. Flat reduction is worded per attack throughout ("reduce damage from any single attack by your Armor", Core Rules Defend), and heavy armor takes "half the sum of the dice" (GM Guide), so splitting into two packets rounds up twice: dice 7 plus bonus 3 becomes 6 rather than 5. Resistance per packet is the reasonable reading; the glossary says only "take half as much damage" and never addresses a mixed-type attack.
