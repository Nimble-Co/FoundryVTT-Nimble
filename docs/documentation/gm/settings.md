...
- **Movement Offers.** When a feature grants a Free Move or pushes a creature, the chat card gets a Move button. Clicking it starts a drag on the owner's client with the ruler limited to the offered distance, so the player still picks the path and the final space. When off, the card shows the distance as plain text and the token is dragged as usual. See [Running Combat](combat.md#movement-offers).
