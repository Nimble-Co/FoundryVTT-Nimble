...
- **Resource Spending.** Mana is deducted when casting, and charges are validated and consumed on use. When off, casting and item use no longer deduct or check costs, so they must be tracked by hand. The spell upcast dialog follows the same rule: with the toggle off it lets you upcast to your highest unlocked tier regardless of the mana on your sheet.
...
- **Movement Offers.** When a feature grants a Free Move or pushes a creature, that creature carries the offer: dragging its token stops the ruler at the offered distance and records the move as a Free Move or as Forced Movement, so it never draws on the creature's own speed. Nothing to click, and the player still picks the path and the final space. When off, nothing is limited or labelled and the card shows the distance as plain text. See [Running Combat](combat.md#movement-offers).
