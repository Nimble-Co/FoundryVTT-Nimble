...
![The Mirefolk ancestry's Rules tab showing the Maximum Wounds rule card](/images/documentation/mirefolk-rules-tab.png)
...
- **Spellcasting**: how this class pays for tiered spells. Leave **Spell Cost Pool** blank and a spell costs its tier in mana, which is the default. Name a charge pool instead and the class spends that pool, with **Cost Per Cast** saying how much. That cost is the same at every tier, so a pool-paying class cannot upcast unless it also casts at its highest tier. **Casting Beyond the Pool** decides what happens on an empty pool: block the cast, or allow it at a cost, and **Consequence Stops After Level** is the last level in this class at which that cost is applied automatically. Levels in other classes do not count. **Cast at Highest Tier** makes every tiered spell resolve at the highest tier the character has unlocked, so the cast window shows the tier instead of offering it. An Upcast Choice spell still opens the cast window, even when the sheet is set to skip it, because the tier is fixed but the enhancement is not: the caster picks it.
...
- **A choice between alternatives** ("choose another Combat Ability, or gain +1 max Combat Dice") is a feature with several level-up options. An option that draws from a group offers that group's pick list; an option that grants an item outright carries a **Grant Item** rule pointing at it. On a rest that offers an option swap, every alternative a level offered is one pool, so the granted item can be traded for a pick from the group and back.
- **A repeatable option** is authored on the granting alternative, not on the item. Turn on **Allow duplicates on the actor** in that option's Grant Item rule, and list every level it applies at. The level-up window grants a fresh copy each time, and the option swap counts every copy on the sheet as its own pick and keeps offering the option while the character already holds one. The number of picks a pool grants comes from the level-up options themselves: each level that offers the pool adds its own count, and a level that offers a choice between alternatives adds one. The flag is off by default, which is the right setting for every "choose a 2nd ability" style pick: the swap then offers a member only while the character holds no pick of it. The Commander's +1 Max Combat Die in the **Class Features** compendium is the shipped example.
...
![A class Progression tab showing level rows, an auto-granted feature, and a choice group](/images/documentation/class-progression-tab.png)
