...
| `spacesMovedThisTurn:0` | Spaces Moved This Turn: how far the creature has moved since its own turn began, counting Regular Movement (a Free Move included) and Forced Movement. A Teleport or a Swap never counts. Present only for combatants in a started combat while the Movement Tracking automation toggle is on; outside those the tag is absent and no `min` or `max` test on it matches. In a started combat, `{ "spacesMovedThisTurn": { "max": 0 } }` reads "has not moved this turn" and `{ "min": 4 }` reads "after moving at least 4 spaces". |
