...
- **Options.** Some features offer themselves on the card instead of waiting on the sheet, and appear together under an **Options** heading. Defensive ones belong to whoever was attacked, such as an Interpose to take a hit for an ally or a reroll to force the attacker to try again. Offensive ones belong to the attacker: a feature that spends dice to add damage, like the Berserker's Death Blow, shows an **Add Damage** button on the card once the attack lands the way the feature needs. Only the player who owns the acting character (and the GM) sees a given button, and each one can be used once per card. Once used, the card records what it did so the table can see where the extra damage came from.
...
## Spending dice from a card

A feature that adds damage when an attack lands puts its button on that attack's card rather than asking you to run it separately. Click it, pick which dice to spend, and confirm: the bonus is added to the attack's own damage total, so the number in the roll box goes up and the GM still clicks **Apply Damage** exactly once.

This matters for more than convenience. Applying the bonus as a second, separate hit would make the target's armor, resistances and damage reduction count twice against one attack, which is why the card folds it in instead. Because the button only appears when the attack landed the way the feature requires, a feature that works only on a critical hit will not offer itself on an ordinary hit.

Spend before the GM applies damage. The card raises its own total, so a spend confirmed after damage has already been applied will not go back and adjust HP that has already been removed.
