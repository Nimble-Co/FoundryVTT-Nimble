...
- **Apply Damage** *(GMs only)*. Applies the damage to the card's targets. If a target has damage reduction, from a rule on an item or a one-shot amount banked by a feature, the reduced number is what lands, and the button's preview shows it. The button stays available even when reduction absorbs the whole hit, and is disabled only when there's nothing to apply, for example on a miss.
