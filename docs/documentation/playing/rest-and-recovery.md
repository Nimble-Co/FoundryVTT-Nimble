...
The Songweaver's Jack of All Trades works the same way for skills: it offers to move a skill point on a safe rest. Under **Move a point**, the card shows one line for each point the feature lets you move, so a feature that moves two points shows two lines. Each line takes a point from one skill and gives it to another, both chosen from a list that shows the current total. A move is net zero, so a point can only be added to one skill by taking it from another. A point can only leave a skill that holds one and whose total would stay at +0 or better, and no skill can be pushed past the +12 maximum. The lines are counted together: a skill can only give as many points as it holds, so a skill with one point can give on one line only. If the feature that offers the move is itself a pick you are giving up in the same rest, its card says so and the moves are not offered.
