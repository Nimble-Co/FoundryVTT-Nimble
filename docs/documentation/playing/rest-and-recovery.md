...
- **Each pool of options** your class has given you picks from. Your current picks sit at the top, marked as selected. The rest of the pool is folded below them behind a **Show more options** row, so a class with many pools stays readable. In a pool with a single pick, clicking another option swaps it in. In a pool with several picks, deselect one of yours first, then pick its replacement. The pool keeps you to the number of picks you are owed.
