 ...
 *
 * A level whose feature offers a choice between alternatives is folded into one requirement
 * naming every group any alternative could draw from. It counts once, whichever way the choice
 * went.
 *
 * Every option is an item on the sheet, so every level is counted. Such a level must not be
 * skipped again: skipping is what hid a deleted pick at that level from the sheet.
 */
