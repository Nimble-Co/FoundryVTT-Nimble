...
describe('ToggleEffectRule — target marking', () => {
	...
	function createRule(
		...
		owner: MarkOwner,
		itemUuid = 'item-uuid',
	): { rule: ToggleEffectRule; item: { uuid: string } } {
		...
		const rule = new ToggleEffectRule(sourceData as never, {
			parent: item as never,
			strict: false,
		});

		rule.type = 'toggleEffect';
		rule.tags = [];
		rule.turnOff = [];
		rule.clearPoolsOnEnd = [];
		rule.endAfterInactiveRounds = null;
		rule.confirmEndPrompt = '';
		...
		rule.durationDays = config.durationDays ?? 1;
	...
	function activation(
		sourceItem: { uuid: string },
		tokens: ReturnType<typeof createTargetToken>[],
	) {
		return {
			sourceItem,
			sourceActor: {},
	...
	it('creates no backing ActiveEffect for a pure marking rule', async () => {
		const owner = createOwner();
		const { rule, item } = createRule({}, owner);
		owner.rules = [rule];
		const token = createTargetToken('goblin', 'Goblin');
	...
	it('replaces the prior quarry at capacity 1', async () => {
		const owner = createOwner();
		owner.getFlag = vi.fn(() => ({
			quarry: [
				{ actorUuid: 'Actor.old', tokenUuid: null, name: 'Old', markedAt: 0, durationDays: null },
			],
		}));
