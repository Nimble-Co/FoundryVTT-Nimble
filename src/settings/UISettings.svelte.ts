...
import UISettingsDialog from '../view/dialogs/UISettingsDialog.svelte';
import { settings } from './settings.js';

const { ApplicationV2 } = foundry.applications.api;

export class UISettings extends SvelteApplicationMixin(ApplicationV2) {
	data: Record<string, any>;

	root = UISettingsDialog;
	...
	constructor(options = {}, _dialogData = {}) {
		...
		super(
			...
			foundry.utils.mergeObject(options, {
				id: 'nimble-ui-settings',
				classes: ['nimble-sheet', 'nimble-sheet--settings'],
				position: { width: 650, height: 'auto' },
				window: { title: 'NIMBLE.settings.uiMenu.name', icon: 'fa-solid fa-cog' },
			}),
		);
	...
	static getActiveApp(): UISettings {
		// @ts-expect-error
		return Object.values(ui.windows).find((app) => app.id === 'nimble-ui-settings');
	}

	static async show(options = {}, dialogData = {}) {
		const app = UISettings.getActiveApp();
		...
		return new Promise((resolve) => {
			...
			new UISettings(options, dialogData).render(true, { focus: true });
		});
	}
