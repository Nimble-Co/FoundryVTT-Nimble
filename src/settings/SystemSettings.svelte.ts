...
export class SystemSettings extends SvelteApplicationMixin(ApplicationV2) {
	...
	constructor(options = {}, _dialogData = {}) {
		...
		super(
			...
			foundry.utils.mergeObject(options, {
				id: 'nimble-system-settings',
				classes: ['nimble-sheet', 'nimble-sheet--settings'],
				position: { width: 650, height: 'auto' },
				window: { title: 'NIMBLE.settings.menu.name', icon: 'fa-solid fa-cog' },
			}),
		);
	...
	static getActiveApp(): SystemSettings {
		// @ts-expect-error
		return Object.values(ui.windows).find((app) => app.id === 'nimble-system-settings');
	}

	static async show(options = {}, dialogData = {}) {
		const app = SystemSettings.getActiveApp();
		...
		return new Promise((resolve) => {
			...
			new SystemSettings(options, dialogData).render(true, { focus: true });
		});
	}
