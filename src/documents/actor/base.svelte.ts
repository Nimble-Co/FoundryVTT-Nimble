...
export type SystemActorTypes = Exclude<
  foundry.documents.BaseActor.TypeNames,
  "base"
>;

// @ts-ignore - Ignoring infinite error
interface NimbleBaseActor<
...
class NimbleBaseActor extends Actor {
  ...
  static override async createDialog<T extends Document.AnyConstructor>(
    this: T,
    data?: DeepPartial<
      Document.ConstructorDataFor<T> & Record<string, unknown>
    >,
    context?: Pick<
      DatabaseCreateOperation<InstanceType<T>>,
      "parent" | "pack"
    > &
      InexactPartial<
        DialogOptions & {
  ...
  isType<TypeName extends SystemActorTypes>(
    type: TypeName,
  ): this is NimbleBaseActor<TypeName> {
  ...
  get sourceId(): string {
    // @ts-expect-error
    return (
      this._stats.compendiumSource || this.flags?.core?.source || undefined
    );
  }
    ...
    if (healingType === "temporaryHealing") {
      if (healing <= temp) {
        ui.notifications.warn(
          "Temporary hit points were not granted to {this.name}. ",
          {
            localize: true,
          },
        );
  ...
  ): Promise<NimbleBaseItem | undefined> {
    const item = this.items.get(id);

    if (!item) {
      // eslint-disable-next-line no-console
      console.error(
        `Attempted to update item with id ${item}, but the item could not be found.`,
      );
      return undefined;
    }
  ...
  async activateItem(
    ...
    if (!item) {
      // eslint-disable-next-line no-console
      console.error(
        `Attempted to activate item with id ${item}, but the item could not be found.`,
      );
      return null;
    }
  ...
  async rollAbilityCheckToChat(
    ...
    const chatData = await this.prepareAbilityCheckChatCardData(
      abilityKey,
      roll,
      {
        ...options,
  ...
  async rollAbilityCheck(
    ...
    const baseRollMode = calculateRollMode(
      this.isType("character")
        ? (this.system.abilities[abilityKey].defaultRollMode ?? 0)
        : 0,
    ...
    if (options.skipRollDialog) {
      rollData = await this.getDefaultAbilityCheckData(
        abilityKey,
        baseRollMode,
        options,
      );
  ...
  async rollSavingThrow(saveKey: saveKey, options: ActorRollOptions = {}) {
    ...
    if (options.skipRollDialog) {
      rollData = await this.getDefaultSavingThrowData(
        saveKey,
        baseRollMode,
        options,
      );
