<script>
    ...
    function getSecondaryInformation(outcome, ignoreArmor, rollMode) {
        ...
        if (outcome === "fullDamage" && ignoreArmor)
            return `${rollModeSummary} (Ignores Armor)`;

        if (outcome === "halfDamage") {
            if (ignoreArmor)
                return `${rollModeSummary} (Half Damage, Ignores Armor)`;
    ...
    let secondaryInfo = $derived(
        getSecondaryInformation(outcome, ignoreArmor, rollMode).trim(),
    );
</script>
