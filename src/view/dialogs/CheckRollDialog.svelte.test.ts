...
/**
 * The other route into the same section: a `savingThrowRollMode` rule that names a
 * situation is skipped by the stored-default calculation, so a save roll is the only
 * place it can be applied. `situationalSaveRules.test.ts` covers the collector itself.
 */
describe('CheckRollDialog situational saving throw rules', () => {
	beforeAll(() => {
		(Math as unknown as { clamp: (n: number, min: number, max: number) => number }).clamp = (
			n,
			min,
	...
	function renderSave(type: 'savingThrow' | 'abilityCheck') {
		const submitRoll = vi.fn();
		render(CheckRollDialog, {
			actor: { rules: [], items: [survivalist()] },
			dialog: { submitRoll },
			type,
			saveKey: 'strength',
			rollMode: 0,
		...
		return {
			submitRoll,
			roll: () => fireEvent.click(screen.getByRole('button', { name: /roll/i })),
			toggle: () => fireEvent.click(screen.getByRole('checkbox', { name: /Survivalist/ })),
		};
	}

	it('offers a situational save rule as a toggle on a saving throw', async () => {
		const { submitRoll, roll, toggle } = renderSave('savingThrow');
	...
	it('offers nothing on a roll that is not a saving throw', () => {
		renderSave('abilityCheck');
