<script lang="ts">
	...
	function prepareItemTooltip(_item: unknown) {
		return null;
	}

	function getFeatureMetadata(_item: unknown) {
		return null;
	}

	function _getMonsterFeatureIcon(categoryName: string) {
		switch (categoryName) {
			case 'bloodied':
	...
	function handleEditorSave(event: Event, updatePath: string, _editState: boolean) {
		const target = event.target as HTMLElement | null;

		if (!target) return;
		if (target.dataset?.action !== 'save') return;

		const editor = target.parentNode?.closest(`[name="${updatePath}"]`) as HTMLElement | null;

		if (editor?.dataset?.documentUUID !== actor.uuid) return;

		_bloodiedEffectInEditMode = false;
		_lastStandEffectInEditMode = false;
	...
	let _features = $derived.by(() =>
		actor.reactive.items.filter((item) => !item.reactive.system.isAction),
	);

	let _actions = $derived.by(() =>
		actor.reactive.items.filter((item) => item.reactive.system.isAction),
	);

	let _bloodiedEffectInEditMode = $state(false);
	let _lastStandEffectInEditMode = $state(false);
			...
			{#each Object.entries(categorizedItems).sort(sortItemCategories) as [categoryName, itemCategory]}
				<section class="nimble-monster-sheet-section">
					...
					{#if categoryName === 'action' && actor.reactive.type === 'soloMonster'}
						...
						{:else}
							<div
								class="nimble-monster-feature-text-with-button"
					...
					>
						{#each sortItems(itemCategory) as item (item.reactive._id)}
							...
							<li
								...
								role="button"
								ondragstart={(event) => {
									event.dataTransfer.setData('nimble/reorder', item._id);
								...
								<header class="u-semantic-only">
									...
									{#if !isCollapsed}
										<button
											...
											onclick={(event) => {
												event.stopPropagation();
												item.update({ 'flags.nimble.collapsed': true });
											}}
										>
									...
									{:else}
										<button
											...
											onclick={(event) => {
												event.stopPropagation();
												item.update({ 'flags.nimble.collapsed': false });
											}}
										>
							...
							{#if !isCollapsed}
								<div
									class="nimble-monster-feature-text"
									style="display: flex; align-items: center; gap: 0.5rem;"
								>
