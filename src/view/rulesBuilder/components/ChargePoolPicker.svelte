<script lang="ts">
	...
	import { createChargePoolPickerState } from './ChargePoolPicker.svelte.js';

	type ChargePoolPickerProps = {
		value: string;
	...
	let {
		...
		document,
		required = false,
	}: ChargePoolPickerProps = $props();

	const state = createChargePoolPickerState(() => document);
